...
The RPM spec file provided here is for Esa-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://esacrypto.com/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Esa git repository](https://github.com/JustAResearcher/Esa) but are not part of
...
Currently only `Source10` is used, for the example `esa.conf` file.
...
in the esa git repository. At present only an SVG version of the Esa
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the esa
...
needed when 0.12.1 is released, a proper fix is already in the Esa git
master branch.
...
idea to build Esa Core against a static version of that library compiled
...
The `esa.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `esa-server` package. At this point, most RPM based
...
The `esa-server` package should have SELinux support. How to properly do
...
Esa-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/esa/). He has been
packaging Esa for Fedora at least since 2012.
...
4. I renamed the `esa` package that contains the Qt GUI to `esa-core` as
that appears to be how the general population refers to it, in contrast to
`esa-xt` or `esa-classic`. I wanted to make sure the general population
