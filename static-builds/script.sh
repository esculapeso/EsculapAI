...
find ${DISTNAME}/bin -type f -executable -exec ../Esa/contrib/devtools/split-debug.sh {} {} {}.dbg \;
find ${DISTNAME}/lib -type f -exec ../Esa/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
cd ~/
rm -rf win64
cp -rf Esa/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir -p unsigned
mv ~/Esa/esa-*setup-unsigned.exe unsigned/
...
cd ~/
rm -rf win32
cp -rf Esa/contrib/windeploy ~/sign/win32
...
mkdir -p ~/Esa/depends/SDKs
cp ~/MacOSX10.11.sdk.tar.gz ~/Esa/depends/SDKs/MacOSX10.11.sdk.tar.gz
cd ~/Esa/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
