...
#if defined(NDEBUG)
# error "Esa cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Esa Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s esa additional fees, %d delta bytes\n",
...
bool GetTransaction(const uint256 &hash, CTransactionRef &txOut, const Consensus::Params& consensusParams, uint256 &hashBlock, bool fAllowSlow)
{
    ...
    if (pindexSlow) {
        CBlock block;
        LogPrintf("ReadBlockFromDisk 1 %s\n");
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight, uint256 blockHash, CAssetsCache* assetCache, std::pair<std::string, CBlockAssetUndo>* undoAssetData)
{
    ...
    if (!tx.IsCoinBase()) {
        ...
        for (const CTxIn &txin : tx.vin) {
            ...
            bool is_spent = inputs.SpendCoin(txin.prevout, &txundo.vprevout.back(), assetCache); /** ESA START */ /* Pass assetCache into function */ /** ESA END */
            assert(is_spent);
        }
    }
    // add outputs
    AddCoins(inputs, tx, nHeight, blockHash, false, assetCache, undoAssetData); /** ESA START */ /* Pass assetCache into function */ /** ESA END */
}
...
//                                      << ", " << i << ", " << hash.GetHex() << ", " << k << ", " << "true" << ", " << assetAmount << std::endl;
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                Coin coin;
                bool is_spent = view.SpendCoin(out, &coin, &tempCache); /** ESA START */ /* Pass assetsCache into the SpendCoin function */ /** ESA END */
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                int res = ApplyTxInUndo(std::move(undo), view, out, assetsCache); /** ESA START */ /* Pass assetsCache into ApplyTxInUndo function */ /** ESA END */
...
//                                      << ", " << i << ", " << txhash.GetHex() << ", " << k << ", " << "true" << ", " << assetAmount << std::endl;
    ...
    if (!IsValidDestination(destCommunityAutonomous)) {
		LogPrintf("IsValidDestination: Invalid Esa address %s \n", GetCommunityAutonomousAddress);
    }
	// Parse Esa address
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    try {
    {
        ...
        if (fDoFullFlush) {
            ...
            if (!CheckDiskSpace((48 * 2 * 2 * pcoinsTip->GetCacheSize()) + assetDirtyCacheSize * 2)) /** ESA START */ /** ESA END */
                return state.Error("out of disk space");
...
bool static DisconnectTip(CValidationState& state, const CChainParams& chainparams, DisconnectedBlockTransactions *disconnectpool)
{
    ...
    CBlock& block = *pblock;
    LogPrintf("ReadBlockFromDisk 2 %s\n");
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrintf("ReadBlockFromDisk 3 %s\n");
...

    {
        ...
        LogPrint(BCLog::BENCH, "  - Flush esa: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime4 - nTime3) * MILLI, nTimeFlush * MICRO, nTimeFlush * MILLI / nBlocksTotal);
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!ConnectBlock(block, state, &indexDummy, viewNew, chainparams, &assetCache, true)) /** ESA START */ /*Add asset to function */ /** esa END*/
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    LogPrintf("ReadBlockFromDisk 4 %s\n");
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            ...
            CBlock block;
    LogPrintf("ReadBlockFromDisk 5 %s\n");
            if (!ReadBlockFromDisk(block, pindex, chainparams.GetConsensus()))
                return error("VerifyDB(): *** ReadBlockFromDisk failed at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
...
static bool RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params, CAssetsCache* assetsCache = nullptr)
{
    ...
    CBlock block;
    LogPrintf("ReadBlockFromDisk 6 %s\n");
...
bool ReplayBlocks(const CChainParams& params, CCoinsView* view)
{
    ...
    while (pindexOld != pindexFork) {
        if (pindexOld->nHeight > 0) { // Never disconnect the genesis block.
            CBlock block;
    LogPrintf("ReadBlockFromDisk 7 %s\n");
...
bool LoadExternalBlockFile(const CChainParams& chainparams, FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    LogPrintf("ReadBlockFromDisk 8 %s\n");
