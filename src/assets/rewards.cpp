...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for esa
    ...
    if (p_rewardSnapshot.strDistributionAsset != "esa") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is esa with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using esa differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "esa") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Esa address (already validated during ownership snapshot creation)
